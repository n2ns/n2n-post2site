...
server.tool(
  'n2ns_create_post',
  'Create an article draft or product guide draft. content values must be Markdown document strings; inline HTML is allowed when useful. title and excerpt values must be plain text. This tool defaults to draft; use n2ns_publish_post to publish.',
...
main().catch((error: unknown) => {
  ...
  console.error(`n2ns-content-mcp failed: ${message}`);
  process.exit(1);
});
